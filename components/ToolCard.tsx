/**
 * ToolCard Component
 * Display tool information with hover effects
 */
...
const iconMap: Record<string, any> = {
  ...
  FiChevronsUp: () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline></svg>
  ),
  FiMinusCircle: () => (
...
export function ToolCard({ tool, variant = 'default' }: ToolCardProps) {
  const IconComponent = iconMap[tool.icon] || iconMap.FiGrid;
  const colorScheme = categoryColors[tool.category] || 'gray';

  if (variant === 'compact') {
    return (
      <NextLink href={`/tools/${tool.slug}`} passHref legacyBehavior>
        <Link
          display="block"
          p={4}
          bg="white"
